...
int query(int v,int lo,int hi,int qlo,int qhi,int x){
	...
	if(lo>=qlo && hi<=qhi){
		int greater=(lower_bound(seg[v].begin(),seg[v].end(),x)-seg[v].begin());
		if(greater==seg[v].size()) return INF;
		else return seg[v][greater];
	}
	int mid=(lo+hi)/2;
	int l=2*v+1,r=2*v+2;
	return min(query(l,lo,mid,qlo,qhi,x),query(r,mid+1,hi,qlo,qhi,x));
}
